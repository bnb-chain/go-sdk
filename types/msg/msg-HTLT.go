...
type HTLTMsg struct {
	...
	SenderOtherChain    types.HexData    `json:"sender_other_chain"`
...
func NewHTLTMsg(from, to types.AccAddress, recipientOtherChain, senderOtherChain []byte, randomNumberHash []byte, timestamp int64,
...
type DepositHTLTMsg struct {
	From      types.AccAddress `json:"from"`
	SwapID    types.HexData    `json:"swap_id"`
	OutAmount types.Coins      `json:"out_amount"`
}

func NewDepositHTLTMsg(from types.AccAddress, swapID []byte, outAmount types.Coins) DepositHTLTMsg {
...
func (msg ClaimHTLTMsg) String() string {
	return fmt.Sprintf("claimHashTimeLock{%v#%v#%v}", msg.From, msg.SwapID, msg.RandomNumber)
}
...
func (msg RefundHTLTMsg) String() string {
	return fmt.Sprintf("refundLockedAsset{%v#%v}", msg.From, msg.SwapID)
}
